namespace('kivi.Part', function(ns) {
  ...
  ns.lemper_set_active = function() {
    var data = $('#ic').serializeArray();
    data.push({ name: 'action', value: 'Part/save_and_set_active' });

    $.post("controller.pl", data, kivi.eval_json_result);
  };

  ns.lemper_set_inactive = function() {
    var data = $('#ic').serializeArray();
    data.push({ name: 'action', value: 'Part/save_and_set_inactive' });
